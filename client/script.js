...
sayHelloButton.addEventListener('mouseout', changeOff)
// PROBLEM 4
/*
...
const ohMy = () => {
      axios.get('http://localhost:3000/animals').then(res => (console.log(res.data)) )
...
const repeatMyParam = (stringHere) => {
    axios.get(`http://localhost:3000/repeat/${stringHere}`)
    .then(response =>{
...
function addQueery(event){
    axios
    .get(`http://localhost:3000/query-test${query}`).then(res =>{
    console.log(res.data)
})
}
document.getElementById('query-button').addEventListener('click', addQueery)
